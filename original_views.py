...
''',
            ...
            {
                'id': 5,
                'title': 'Создание сайта Django',
                'description': 'Пошаговое создание учебного сайта на Django и наполнение содержимым.',
                'presentations': [
                    {
                        'title': 'Учебный сайт на Django',
                        'filename': 'Django_Учебный_сайт.pdf',
                    },
                ],
                'info': '''
<h3>Создание сайта Django</h3>
<p>В этой теме показано, как создать учебный сайт на Django и перенести в него весь учебный контент.</p>

<h3>1. Создание проекта и приложения</h3>
<p>Запустите команды:</p>
<pre>django-admin startproject my_project .
python manage.py startapp main</pre>

<h3>2. Настройте приложение</h3>
<p>Добавьте <code>main</code> в <code>INSTALLED_APPS</code> в <code>my_project/settings.py</code>. Настройте <code>STATIC_URL</code>, <code>MEDIA_URL</code>, <code>STATICFILES_DIRS</code>, <code>MEDIA_ROOT</code>.</p>

<h3>3. Сделайте маршруты</h3>
<p>Опишите путь для списка модулей, страницы модуля и темы. В <code>my_project/urls.py</code> добавьте:</p>
<pre>path('', module_list, name='module_list')
path('modules/<int:module_id>/', module_detail, name='module_detail')
path('modules/<int:module_id>/topics/<int:topic_id>/', topic_detail, name='topic_detail')</pre>

<h3>4. Сделайте админку</h3>
<p>Зарегистрируйте модели <code>Module</code>, <code>Topic</code>, <code>Presentation</code> и <code>TopicImage</code> в <code>main/admin.py</code>, чтобы управлять содержимым из браузера.</p>

<h3>5. Наполнение сайта</h3>
<p>Добавьте в админке модули, темы, презентации и изображения. Это позволяет изменять текст и медиа без правки кода.</p>

<h3>6. Проверка</h3>
<p>Запустите сервер:</p>
<pre>python manage.py runserver</pre>
<p>Откройте сайт и убедитесь, что ваш учебный контент доступен.</p>
''',
                'images': [
                    {
                        'src': settings.STATIC_URL + 'main/images/1.21.png',
                        'caption': 'Учебный сайт Django и структура проекта.',
                    },
                ],
            },
        ],
